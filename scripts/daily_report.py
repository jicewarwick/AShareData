...
if __name__ == '__main__':
    ...
    model_reader = asd.ModelDataReader()
    model_factor_ret = model_reader.model_factor_return.bind_params(ids=['FF3_SMB_DD', 'FF3_HML_DD', 'Carhart_UMD_DD'])
    date = dt.datetime.combine(dt.date.today(), dt.time())
    print('')
    print('因子收益率:')
    stock_reader = asd.StockDataReader()
    print(pd.concat([stock_reader.market_return.get_data(dates=date), model_factor_ret.get_data(dates=date)]))
