...
from ..ashare_data_reader import FundDataReader, IndexDataReader
...
class FundNAVAnalysis(object):
    def __init__(self, ticker: str, db_interface: DBInterface = None):
        ...
        self.fund_data_reader = FundDataReader(self.db_interface)
    ...
    def compute_correlation(self, index_code: str, period: int = 60) -> float:
        index_return_factor = self.index_data_reader.get_index_return_factor(index_code)
        start_date = self.fund_data_reader.calendar.offset(dt.date.today(), -period)
