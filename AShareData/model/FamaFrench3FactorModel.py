...
class FamaFrench3FactorModel(FinancialModel):
    ...
    def __init__(self, db_interface: DBInterface = None):
        ...
        policy = StockSelectionPolicy(ignore_st=True, ignore_pause=True, ignore_new_stock_period=244)
    ...
    def update_daily_factor_return(self):
        start_date = self.db_interface.get_latest_timestamp(self.db_table_name, self.start_date,
                                                            column_condition=('ID', self.factor_names[0]))
        ...
        with tqdm(dates) as pbar:
            for date in dates:
                pbar.set_description(f'更新 {self.model_name} 因子日收益率: {date}')
    ...
    def compute_daily_factor_return(self, date) -> pd.Series:
        ...
        bm = self.bm.get_data(ids=tickers, dates=date)
        cap = self.cap.get_data(ids=tickers, dates=date)
        returns = self.returns.get_data(ids=tickers, dates=[pre_date, date])
        df = pd.concat([returns, bm, cap], axis=1).dropna()
        df['G_SMB'] = pd.qcut(df[self.cap.name], self.smb_threshold, labels=['small', 'big'])
        df['G_HML'] = pd.qcut(df[self.bm.name], self.hml_threshold, labels=['low', 'mid', 'high'])
